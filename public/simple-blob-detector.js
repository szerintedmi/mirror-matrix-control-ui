...
// This fallback mirrors OpenCV SimpleBlobDetector behavior for OpenCV.js builds
// that omit the native detector. It exposes `SimpleBlobDetectorFallback` on the
// worker global so we can import it alongside cv.js.
...
(function () {
    ...
    function findBlobs(image, binaryImage, params) {
        ...
        for (let i = 0; i < contours.size(); i++) {
            ...
            if (params.filterByColor) {
                if (
                    binaryImage.ucharAt(
                        Math.round(center.location.y),
                        Math.round(center.location.x),
                    ) !== params.blobColor
                ) {
                    continue;
                }
            ...
            if (!params.faster) {
                ...
                dists.sort();
                center.radius =
                    (dists[Math.floor((dists.length - 1) / 2)] +
    ...
    function simpleBlobDetector(image, params = {}) {
        ...
        let centers = [];
        for (
            let thresh = options.minThreshold;
            thresh < options.maxThreshold;
            thresh += options.thresholdStep
        ) {
            ...
            let curCenters = findBlobs(image, binaryImage, options);
            binaryImage.delete();
            const newCenters = [];
            ...
            for (let i = 0; i < curCenters.length; i++) {
                ...
                for (let j = 0; j < centers.length; j++) {
                    ...
                    isNew =
                        dist >= options.minDistBetweenBlobs &&
        ...
        for (let i = 0; i < centers.length; i++) {
            if (centers[i].length < options.minRepeatability) continue;
    ...
    self.SimpleBlobDetectorFallback = simpleBlobDetector;
})();
