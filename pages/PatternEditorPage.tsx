...
const PatternEditorPage: React.FC<PatternEditorPageProps> = ({
    ...
    const handleClear = () => {
        if (window.confirm('Are you sure you want to clear all pixels? This action cannot be undone.')) {
            setLitPixels(new Set());
        }
    };
        ...
        <div className="flex flex-col h-screen p-4 sm:p-6 lg:p-8">
            <header className="mb-2 flex-shrink-0">
                <div className="flex flex-wrap justify-between items-center gap-4">
                    <button onClick={() => navigation.navigateTo('library')} className="px-4 py-2 rounded-md bg-gray-700 text-gray-200 hover:bg-gray-600 transition-colors border border-gray-600">
                    ...
                    <button onClick={handleSave} className="px-6 py-2 rounded-md bg-cyan-600 text-white font-semibold hover:bg-cyan-500 transition-colors">
                        Save Pattern
                    </button>
                </div>
            </header>
            ...
            <div className="flex justify-center my-4">
                <div className="text-center bg-gray-800/50 ring-1 ring-white/10 p-3 rounded-lg w-full max-w-xs shadow-md">
                    ...
                    <p className={`font-mono text-2xl font-bold mt-1 transition-colors ${pixelCountError ? 'text-red-500' : 'text-cyan-300'}`}>
            ...
            <div className="flex-grow flex flex-col md:flex-row gap-6 min-h-0">
                ...
                <aside className="w-full md:w-72 lg:w-80 bg-gray-800/50 rounded-lg p-4 ring-1 ring-white/10 flex-shrink-0 flex flex-col gap-6">
                    <div>
                        <label htmlFor="patternName" className="block text-sm font-medium text-gray-300">Pattern Name</label>
                        <input
                            type="text"
                            id="patternName"
                            value={name}
                     ...
                     <div className="space-y-4">
                        ...
                        <div className="flex items-center gap-2">
                           <label htmlFor="canvasRows" className="font-medium text-gray-400 w-12">Rows:</label>
                           <input
                               type="number" id="canvasRows" value={canvasSize.rows}
                       ...
                       <div className="flex items-center gap-2">
                           <label htmlFor="canvasCols" className="font-medium text-gray-400 w-12">Cols:</label>
                           <input
                               type="number" id="canvasCols" value={canvasSize.cols}
                    ...
                    <div className="space-y-3 mt-auto">
                        ...
                        <div className="grid grid-cols-3 items-center justify-items-center gap-2 p-2 bg-black/20 rounded-md">
                            <div></div>
                            <button onClick={() => handleShift('up')} className="p-2 rounded-md hover:bg-gray-600 text-gray-300 hover:text-white transition-colors" aria-label="Shift all pixels up">
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a.75.75 0 01-.75-.75V4.66L7.03 6.91a.75.75 0 01-1.06-1.06l3.5-3.5a.75.75 0 011.06 0l3.5 3.5a.75.75 0 01-1.06 1.06L10.75 4.66v12.59A.75.75 0 0110 18z" clipRule="evenodd" /></svg>
                            </button>
                            <div></div>
                            <button onClick={() => handleShift('left')} className="p-2 rounded-md hover:bg-gray-600 text-gray-300 hover:text-white transition-colors" aria-label="Shift all pixels left">
                               <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M2 10a.75.75 0 01.75-.75h12.59l-2.22-2.22a.75.75 0 111.06-1.06l3.5 3.5a.75.75 0 010 1.06l-3.5 3.5a.75.75 0 11-1.06-1.06l2.22-2.22H2.75A.75.75 0 012 10z" clipRule="evenodd" transform="rotate(180 10 10)" /></svg>
                            </button>
                             <button onClick={handleClear} className="p-2 rounded-full hover:bg-red-800/50 text-gray-300 hover:text-red-400 transition-colors" aria-label="Clear all pixels">
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M5 10a1 1 0 011-1h8a1 1 0 110 2H6a1 1 0 01-1-1z" clipRule="evenodd" /></svg>
                            </button>
                            <button onClick={() => handleShift('right')} className="p-2 rounded-md hover:bg-gray-600 text-gray-300 hover:text-white transition-colors" aria-label="Shift all pixels right">
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M2 10a.75.75 0 01.75-.75h12.59l-2.22-2.22a.75.75 0 111.06-1.06l3.5 3.5a.75.75 0 010 1.06l-3.5 3.5a.75.75 0 11-1.06-1.06l2.22-2.22H2.75A.75.75 0 012 10z" clipRule="evenodd" /></svg>
                            </button>
                            <div></div>
                            <button onClick={() => handleShift('down')} className="p-2 rounded-md hover:bg-gray-600 text-gray-300 hover:text-white transition-colors" aria-label="Shift all pixels down">
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 2a.75.75 0 01.75.75v12.59l2.22-2.22a.75.75 0 111.06 1.06l-3.5 3.5a.75.75 0 01-1.06 0l-3.5-3.5a.75.75 0 111.06-1.06l2.22 2.22V2.75A.75.75 0 0110 2z" clipRule="evenodd" /></svg>
                            </button>
                            <div></div>
                        </div>
                    </div>
                ...
                {/* Canvas */}
                <main ref={mainContainerRef} className="flex-grow bg-gray-800/50 rounded-lg ring-1 ring-white/10 grid place-items-center p-4 overflow-hidden">
                    <div
                        onMouseLeave={() => { setIsDrawing(false); setPaintMode(null); }}
