...
const SimulationPreview: React.FC<Omit<SimulationPageProps, 'navigation' | 'gridSize'>> = (props) => {
    const { wallDistance, horizontalAngle, verticalAngle, lightAngleHorizontal, lightAngleVertical } = props;
    ...
    const renderView = (isTopView: boolean) => {
        ...
        const wallY1 = (isTopView ? viewBoxHeight/2 : groundY) + (wallLen / 2) * Math.cos(wallRad);
        const wallX2 = wallBaseX + (wallLen / 2) * Math.sin(wallRad);
        const wallY2 = (isTopView ? viewBoxHeight/2 : groundY) - (wallLen / 2) * Math.cos(wallRad);

        // Line-line intersection
        const x1 = mirrorX, y1 = mirrorY;
        const x2 = endX, y2 = endY;
        const x3 = wallX1, y3 = wallY1;
        const x4 = wallX2, y4 = wallY2;
        ...
        return (
             <div className="relative w-full h-full bg-gray-900/50 rounded-md border border-gray-700 overflow-hidden">
                <span className="absolute top-2 left-3 text-xs text-gray-500">{isTopView ? 'Top View' : 'Side View'}</span>
                <svg viewBox={`0 0 ${viewBoxWidth} ${viewBoxHeight}`} className="w-full h-full">
                    {/* Ground Line */}
                    {!isTopView && <line x1="0" y1={groundY} x2={viewBoxWidth} y2={groundY} stroke="#4A5568" strokeWidth="1" />}
                    
                    {/* Incoming Ray */}
                    <line x1={sourceX} y1={sourceY} x2={mirrorX} y2={mirrorY} stroke="#FBBF24" strokeWidth="2" strokeDasharray="4 4" />
                    <circle cx={sourceX} cy={sourceY} r="5" fill="#FBBF24" />

                    {/* Mirror */}
                    <line x1={mirrorX} y1={mirrorY - mirrorSize/2} x2={mirrorX} y2={mirrorY + mirrorSize/2} stroke="#22D3EE" strokeWidth="4" strokeLinecap="round" />

                    {/* Reflected Ray */}
                    <line x1={mirrorX} y1={mirrorY} x2={endX} y2={endY} stroke="#FBBF24" strokeWidth="2.5" />

                    {/* Wall */}
                    <line x1={wallX1} y1={isTopView ? wallY1 : 0} x2={wallX2} y2={isTopView ? wallY2 : groundY} stroke="#E5E7EB" strokeWidth="3" strokeLinecap="round"/>
                </svg>
            </div>
        );
    };
        ...
        <div className="flex flex-col h-screen bg-gray-900 text-gray-200">
          <div className="flex-shrink-0 p-4 sm:p-6 lg:p-8 pb-0">
             <header className="flex flex-wrap justify-between items-center gap-4">
                  <div>
                      <h1 className="text-4xl font-bold text-cyan-400 tracking-tight">Simulation</h1>
                      <p className="text-gray-400 mt-1">Adjust parameters to see how patterns are projected.</p>
                  </div>
          ...
          <main className="flex-grow flex md:flex-row flex-col gap-8 min-h-0 p-4 sm:p-6 lg:p-8">
              <aside className="w-full md:w-80 lg:w-96 bg-gray-800/50 rounded-lg p-6 shadow-lg ring-1 ring-white/10 flex flex-col gap-4 overflow-y-auto flex-shrink-0">
                  ...
                  <div className="space-y-5">
                      <div>
                          <h3 className="text-lg font-semibold text-gray-200 mb-3">Wall & Projection</h3>
                           <SliderControl label="Wall Distance" value={sliderProps.wallDistance} onChange={sliderProps.onWallDistanceChange} min={1} max={20} step={0.1} unit="m" />
                      </div>
                      <div>
                          <h3 className="text-md font-medium text-gray-300 mb-3">Wall Orientation</h3>
                           <SliderControl label="Horizontal Angle" value={sliderProps.horizontalAngle} onChange={sliderProps.onHorizontalAngleChange} min={-45} max={45} step={0.5} unit="°" />
                           <SliderControl label="Vertical Angle" value={sliderProps.verticalAngle} onChange={sliderProps.onVerticalAngleChange} min={-45} max={45} step={0.5} unit="°" />
                      </div>
                      <div>
                           <h3 className="text-lg font-semibold text-gray-200 mb-3">Light Source</h3>
                           <SliderControl label="Horizontal Angle" value={sliderProps.lightAngleHorizontal} onChange={sliderProps.onLightAngleHorizontalChange} min={-45} max={45} step={0.5} unit="°" />
                           <SliderControl label="Vertical Angle" value={sliderProps.lightAngleVertical} onChange={sliderProps.onLightAngleVerticalChange} min={-45} max={45} step={0.5} unit="°" />
                      </div>
                  </div>

                  <div className="mt-auto pt-5 border-t border-gray-700/50">
                    <h3 className="text-lg font-semibold text-gray-200 mb-3">Projected Pattern Size</h3>
                    <div className="bg-gray-900/70 p-3 rounded-md space-y-2 text-sm">
                        <div className="flex justify-between items-center">
                            ...
                            <span className="font-mono text-cyan-300 text-base">
                                {projectedSize.width !== null ? `${projectedSize.width.toFixed(2)} m` : 'Infinite'}
                            </span>
                        </div>
                        <div className="flex justify-between items-center">
                            <span className="text-gray-400">Est. Height:</span>
                            <span className="font-mono text-cyan-300 text-base">
                                {projectedSize.height !== null ? `${projectedSize.height.toFixed(2)} m` : 'Infinite'}
                            </span>
                        </div>
