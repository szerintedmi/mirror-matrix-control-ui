...
const PatternPreview: React.FC<{ pattern: Pattern }> = ({ pattern }) => {
    ...
    return (
        <div style={containerStyle}>
            <div className="absolute top-0 left-0 w-full h-full grid gap-px bg-gray-800" style={{ gridTemplateColumns: `repeat(${cols}, 1fr)`, gridTemplateRows: `repeat(${rows}, 1fr)` }}>
                {Array.from({ length: rows * cols }).map((_, i) => {
                    const row = Math.floor(i / cols);
                    const col = i % cols;
                    const isLit = litPixels.has(`${row}-${col}`);
                    return <div key={`${row}-${col}`} className={isLit ? 'bg-cyan-400' : 'bg-gray-700/50'} />;
                })}
            </div>
        </div>
    );
    ...
    return (
        <div className="flex flex-col h-screen p-4 sm:p-6 lg:p-8">
            <header className="mb-6 flex flex-wrap justify-between items-center gap-4 flex-shrink-0">
                <div>
                    <h1 className="text-4xl font-bold text-cyan-400 tracking-tight">Pattern Library</h1>
                    <p className="text-gray-400 mt-1">Manage your light patterns and configure the simulation.</p>
                </div>
                <div className="flex items-center gap-4">
                    <button onClick={() => navigation.navigateTo('configurator')} className="px-4 py-2 rounded-md bg-gray-700 text-gray-200 hover:bg-gray-600 transition-colors border border-gray-600">
                        Configure Array
                    </button>
                     <button onClick={() => navigation.navigateTo('simulation')} className="px-4 py-2 rounded-md bg-indigo-600 text-white font-semibold hover:bg-indigo-500 transition-colors">
                        Go to Simulation
                    </button>
                    <button onClick={() => navigation.editPattern(null)} className="px-4 py-2 rounded-md bg-cyan-600 text-white font-semibold hover:bg-cyan-500 transition-colors">
                        Create New Pattern
                    </button>
                </div>
            </header>
            ...
            <main className="flex-grow bg-gray-800/50 rounded-lg p-4 shadow-lg ring-1 ring-white/10 flex flex-col min-h-0">
                <h2 className="text-2xl font-semibold text-gray-100 mb-4 flex-shrink-0">Saved Patterns</h2>
                    ...
                    {patterns.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
                            {patterns.map(pattern => {
                                ...
                                return (
                                <div key={pattern.id} className="bg-gray-900/50 rounded-lg overflow-hidden border border-gray-700 hover:border-cyan-500 transition-colors group flex flex-col">
                                    <div className="p-1"><PatternPreview pattern={pattern} /></div>
                                    <div className="p-3 flex flex-col flex-grow">
                                        <h3 className="font-semibold text-gray-200 truncate">{pattern.name}</h3>
                                        <p className="text-sm text-gray-400 font-mono">{pattern.canvasSize.rows}x{pattern.canvasSize.cols} - {pattern.litPixels.size} pixels</p>

                                        <div className="mt-2 pt-2 border-t border-gray-700/50 text-xs">
                                            <p className="text-gray-400 mb-1">Est. Projection (WxH @ Dist):</p>
                                            <p className="font-mono text-cyan-400 text-sm">{projectedSize.width} &times; {projectedSize.height} @ {projectedSize.distance}</p>
                                        </div>

                                        <div className="mt-3 flex justify-end gap-2 mt-auto">
                                            <button onClick={() => navigation.editPattern(pattern.id)} className="text-sm px-3 py-1 rounded-md bg-gray-700 text-gray-200 hover:bg-gray-600 transition-colors">Edit</button>
                                            <button onClick={() => { if (window.confirm(`Delete "${pattern.name}"?`)) onDeletePattern(pattern.id);}} className="text-sm px-3 py-1 rounded-md bg-red-800/70 text-red-200 hover:bg-red-700/80 transition-colors">Delete</button>
                                        </div>
                                    </div>
                                </div>
                            )})}
