...
const ConfiguratorPage: React.FC<ConfiguratorPageProps> = ({ navigation, gridSize, onGridSizeChange }) => {
    ...
    const isMotorAssigned = useCallback((motor: Motor) => {
        for (const assignment of mirrorConfig.values()) {
            if (assignment.x?.nodeMac === motor.nodeMac && assignment.x?.motorIndex === motor.motorIndex) return true;
            if (assignment.y?.nodeMac === motor.nodeMac && assignment.y?.motorIndex === motor.motorIndex) return true;
        }
        return false;
    }, [mirrorConfig]);
    ...
    const unassignMotor = useCallback((motor: Motor) => {
        setMirrorConfig(prevConfig => {
            ...
            for (const key of newConfig.keys()) {
                ...
                if (newAssignment.y?.nodeMac === motor.nodeMac && newAssignment.y?.motorIndex === motor.motorIndex) {
    ...
    const handleMotorDrop = useCallback((pos: GridPosition, axis: Axis, dragDataString: string) => {
        ...
        setMirrorConfig(prevConfig => {
            ...
            for (const key of newConfig.keys()) {
                ...
                if (newAssignment.y?.nodeMac === motorToMove.nodeMac && newAssignment.y?.motorIndex === motorToMove.motorIndex) {
                    newAssignment.y = null;
                    assignmentChanged = true;
                }
    ...
    const handleUnassignByDrop = useCallback((dragDataString: string) => {
        const dragData: DraggedMotorInfo = JSON.parse(dragDataString);
  ...
  (Simulating move command for mirror at [${pos.row}, ${pos.col}] on axis ${axis.toUpperCase()})`);
             ...
             console.warn(`No motor assigned to axis ${axis.toUpperCase()} for mirror at [${pos.row}, ${pos.col}]`);
        }
    };
        ...
        <>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50" aria-modal="true" role="dialog">
                    <div className="bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-md border border-gray-700">
                        ...
                        <div className="mt-6 flex justify-end space-x-4">
                            <button onClick={() => setModalState({ ...modalState, isOpen: false })} className="px-4 py-2 rounded-md bg-gray-600 text-white font-semibold hover:bg-gray-500 transition-colors focus:outline-none focus:ring-2 focus:ring-gray-400">Cancel</button>
                            <button onClick={modalState.onConfirm} className="px-4 py-2 rounded-md bg-red-600 text-white font-semibold hover:bg-red-500 transition-colors focus:outline-none focus:ring-2 focus:ring-red-400">Confirm</button>
                        </div>
                    </div>
                </div>
            )}
            <div className="min-h-screen bg-gray-900 text-gray-200 flex flex-col p-4 sm:p-6 lg:p-8">
                 <header className="mb-6 flex flex-wrap justify-between items-start gap-4">
                    <div>
                        <h1 className="text-4xl font-bold text-cyan-400 tracking-tight">Mirror Array Configurator</h1>
                        <p className="text-gray-400 mt-1">Visually assign motors to mirrors and test your configuration.</p>
                    </div>
                    <div className="flex items-center gap-4">
                        ...
                        <button onClick={handleResetAll} className="flex items-center gap-2 px-4 py-2 rounded-md bg-red-800/70 text-red-200 hover:bg-red-700/80 transition-colors border border-red-600/80" title="Reset all assignments">
                         ...
                         <div className="flex flex-wrap justify-between items-center gap-4 mb-4">
                            ...
                            <div className="flex items-center gap-4">
                               <div className="flex items-center gap-2">
                                  <input type="checkbox" id="unassignedFilter" checked={showOnlyUnassigned} onChange={(e) => setShowOnlyUnassigned(e.target.checked)} className="h-4 w-4 rounded bg-gray-700 border-gray-600 text-cyan-500 focus:ring-cyan-600" />
                                  <label htmlFor="unassignedFilter" className="text-sm text-gray-300">Show only unassigned</label>
                               </div>
                                <button onClick={fetchNodes} className="p-2 rounded-full hover:bg-gray-700 transition-colors" title="Rediscover Nodes">
                                <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 ${isLoading ? 'animate-spin' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h5M20 20v-5h-5M4 4a8 8 0 0113.52 4.857M20 20a8 8 0 01-13.52-4.857" />
                                    </svg>
                                </button>
                            </div>
                        </div>
