...
interface PatternLibraryListProps {
    ...
    onPlay?: (pattern: Pattern) => void;
    enableDragAdd?: boolean;
    onPrimaryClick?: (pattern: Pattern) => void;
    disablePrimaryClick?: boolean;
    ...
    return (
        <ul className={`space-y-2 overflow-y-auto pr-2 ${className}`}>
            {patterns.map((pattern) => {
                const isSelected = !suppressSelectionHighlight && selectedPatternId === pattern.id;
                const validation = getValidationStatus ? getValidationStatus(pattern) : null;
                const isInvalid = validation && !validation.isValid;
                const handleCardClick = () => {
                    if (disablePrimaryClick) return;
                    if (onPrimaryClick) {
                        onPrimaryClick(pattern);
                        return;
                    }
                    onSelect(pattern.id);
                };
                ...
                return (
                    <li key={pattern.id}>
                        <div
                            role="button"
                            tabIndex={0}
                            draggable={enableDragAdd || Boolean(onAdd)}
                            className={`group relative flex w-full cursor-pointer items-center gap-3 rounded-md border p-3 text-left transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500 focus-visible:ring-offset-2 focus-visible:ring-offset-gray-900 ${
                                isSelected
                            ...
                            onDragStart={(event) => {
                                if (!(enableDragAdd || onAdd)) return;
                                event.dataTransfer.setData('application/x-pattern-id', pattern.id);
                            ...
                            <div className="flex flex-none items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                                {onAdd && (
                                    <button
                                        ...
                                        className="rounded p-1.5 text-emerald-200 hover:bg-emerald-900/40 hover:text-white"
                                ...
                                {onDelete && (
                                    <button
                                        ...
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            if (
                                                window.confirm(`Delete pattern "${pattern.name}"?`)
                                            ) {
                                                onDelete(pattern.id);
                                            }
                                        }}
