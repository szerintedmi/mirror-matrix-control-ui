import type { LegacyPattern, LegacyPatternCanvas, LegacyPatternTile } from '../types';
...
const parseCanvas = (input: unknown): LegacyPatternCanvas | null => {
    if (!input || typeof input !== 'object') {
        return null;
    }
    const candidate = input as Partial<LegacyPatternCanvas>;
...
const parseTile = (input: unknown): LegacyPatternTile | null => {
    if (!input || typeof input !== 'object') {
        return null;
    }
...
const parsePattern = (input: unknown): LegacyPattern | null => {
    if (!input || typeof input !== 'object') {
        return null;
    }
...
export const loadLegacyPatterns = (storage: Storage | undefined): LegacyPattern[] => {
    if (!storage) {
        return [];
    }
...
const writeLegacyPatterns = (storage: Storage | undefined, patterns: LegacyPattern[]): void => {
    if (!storage) {
        return;
    }
...
export const persistLegacyPatterns = (
    storage: Storage | undefined,
    patterns: LegacyPattern[],
): void => {
    writeLegacyPatterns(storage, patterns);
};

export const removeLegacyPattern = (storage: Storage | undefined, patternId: string): void => {
    if (!storage) {
        return;
    }
    const existing = loadLegacyPatterns(storage);
    const next = existing.filter((pattern) => pattern.id !== patternId);
    writeLegacyPatterns(storage, next);
};
