...
import type { ArrayRotation, BlobMeasurement, MirrorConfig, Motor } from '@/types';

function createTestRunner(arrayRotation?: ArrayRotation) {
    ...
    const captureMeasurement = vi.fn().mockImplementation(async () => measurements.shift() ?? null);
    ...
    return { completion, moveCalls, xMotor };
}

describe('CalibrationRunner axis directions', () => {
    it('at 0° rotation, stages X to the positive side', async () => {
        const { completion, moveCalls, xMotor } = createTestRunner(0);
        await completion;
    ...
    it('at 180° rotation, stages X to the negative side', async () => {
        const { completion, moveCalls, xMotor } = createTestRunner(180);
        await completion;
